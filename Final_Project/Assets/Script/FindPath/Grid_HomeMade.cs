...
public class Grid_HomeMade : MonoBehaviour
{
    ...
    public int[,] gridMatrixForAMap(Tilemap tilemap)
    {
        ...
        for (int row = 0; row < numRows; row++)
        {
            for (int col = 0; col < numCols; col++)
            {
                ...
                Vector3Int cellPosition = grid.WorldToCell(worldPosition); // Chỗ này fix bug
    ...
    //             // Chuyển đổi sang tọa độ góc dưới-trái của mỗi ô
    //             float adjustedX = worldX - cellSize.x / 2;
    //             float adjustedY = worldY - cellSize.y / 2;

    //             worldCoordinates[row, col] = new Vector3(adjustedX, adjustedY, 0);
    //         }
    //     }
    ...
    //     // Chuyển sâng tọa độ 
    //     Vector3[,] coordinates = changeTocoordinates(grid_test);
    //     // Gán ô mục tiêu để vẽ

    //     // Example: find the path
    //     Debug.Log(toString(coordinates));
    //     Debug.Log(CheckForTile(tilemap[1], new Vector3Int(0,-9,0)));
    //     Debug.Log(CheckForTile(tilemap[0], new Vector3Int(3,-9,0)));
    // }
}
